...
import { DeckPathSegment, getPublishedFilePathSegments, getPublishedDeckByName } from "../../lib/api";
import { ParsedUrlQuery } from "querystring";
import { GetStaticProps, GetStaticPaths } from "next";
import * as path from "path";
import Flashcard from "../../components/card";
import { downloadLatestRelease,  getLatestRelease, getRepo } from "../../services/gitService";
import { Layout } from "../../components/layout";
import { getCommunityDeckListPathSegments } from '../../lib/api';
import { GitHubComments } from '../../components/github-comments';
import { log } from "fp-ts/lib/Console";
import { decksBaseDir, fullDeckDataPath, fullReleaseDataPath, fullRepoDataPath, writeFile } from "../../lib/filesystem";
...
export default function DeckPage({ Odeck }: DeckPageProps) {
  if (O.isNone(Odeck)) {
    ...
    return (
      <Layout meta={<Meta title={deck.deck.title} desc={O.none} canonical="TODO" />}>
...
const updateRepoInfo = async ({author, deck}: DeckPathSegment) => {
 ...
 await F.pipe(
  ...
  if (E.isLeft(maybeLatestRelease)) {
	...
	const maybeDeck = await downloadLatestRelease(deckAsset.browser_download_url)();
	if (E.isLeft(maybeDeck)) {
	  ...
	  await Promise.all([writeReleaseInfo(release, author, deck)(), writeLatestDeck(deckdata, author, deck)])
	}
  }
}
