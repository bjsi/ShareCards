...
const updateRepoInfo = async ({ author, deck }: DeckPathSegment) => {
  ...
  if (E.isLeft(createDirRes)) {
    console.log(`Failed to create deck directory: ${author}/${deck} with error: ${createDirRes.left.message}`);
    return;
  }
  ...
  if (E.isLeft(mrelease)) {
    console.log(`Failed to get latest release info for ${author}/${deck} with error: ${mrelease.left.message}`);
    return;
  } else {
    ...
    if (!asset) {
      console.log(`Latest release for for ${author}/${deck} contains no deck.json asset`);
      return;
    }

    const mdeck = await downloadLatestRelease(
