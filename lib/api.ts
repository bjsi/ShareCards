...
import { communityDeckList,fullRepoDataPath, fullDeckDataPath, fullReleaseDataPath, decksBaseDir, deckFileName, readFile, fullAuthorBasePath } from "./filesystem";
 ...
 */
export const getCommunityDeckListPathSegments = (): T.Task<DeckPathSegment[]> => {
  return F.pipe(
    readAndDecodeJson(communityDeckList, t.array(t.string)),
    TE.fold(
      err => {console.log(err); return T.of([])},
      decks => F.pipe(
	decks,
        A.map(x => x.split('/')),
 ...
 */
export const getPublishedFilePathSegments = (startingDir: string): DeckPathSegment[] => {
  return F.pipe(
...
const readPublishedDeck = ({author, deck}: DeckPathSegment): TE.TaskEither<Error, PublishedDeck> => {
  return F.pipe(
     {
       repo: readAndDecodeJson(fullRepoDataPath(author, deck), repoData),
       deck: readAndDecodeJson(fullDeckDataPath(author, deck), deckData),
       release: readAndDecodeJson(fullReleaseDataPath(author, deck), releaseData),
     },
 ...
 */
export const getPublishedDeckByName = (segments: DeckPathSegment): T.Task<O.Option<PublishedDeck>> => {
  return F.pipe(
    readPublishedDeck(segments),
    TE.fold(
	e => {console.log(e); return T.of(O.none);},
	deck => T.of(O.some(deck))
    )
  )
}

const readPublishedDecks = (segments: DeckPathSegment[]): T.Task<PublishedDeck[]> => {
	return F.pipe(
			segments,
 ...
 * an invalid deck will be logged and the deck in question will be excluded from the final list.
 */
export const getPublishedDecksByAuthor = (author: string): T.Task<PublishedDeck[]> => {
  return F.pipe(
