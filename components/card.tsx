...
import { ShowAt, ShowState, htmlComponent, ImageComponent, SoundComponent } from '../models/components';
...
const HtmlComponent = ({showState, showAt, text}: HtmlCompProps) => {
	  	return (
			<Card border="dark" style={{display:shouldHideComponent(showState, showAt)}} className="m-1">
				<Card.Body>
...
const Item = ({data, showState}: ItemProps) => {
	...
	<>
		  ...
		  data.comps.map((comp, idx) => 
		  	<HtmlComponent {...{key:idx, showState, showAt: comp.showAt, text: comp.text}} />)
		}
	</>
	)
}
...
const ElementButton = ({showState, children, clickHandler, showAt}: ElementButtonProps) => {
	return showAt & showState 
		? <Button  className="m-1" onClick={clickHandler}>{children}</Button>
		: null 
}

const TestRepetitionButton = ({setState, showState}: AnswerButtonsProps) => {
	const clickHandler = () => setState((_) => ShowState.Question)
	return <ElementButton {...{showState, showAt: ShowState.Browsing}} clickHandler={clickHandler}>
		Test Repetition
		</ElementButton>;
}
...
const ShowAnswerButton = ({setState, showState}: AnswerButtonsProps) => {
  ...
  return <ElementButton {...{showState, showAt: ShowState.Question}} clickHandler={clickHandler}>Show Answer</ElementButton>
}
...
export default function Flashcard({data, isPreview}: FlashcardProps) { 
  ...
  const element = () => {
  	switch (data.template) {
		case "Item": return <Item {...{showState, data}}/>;
		case "Item Picture": return <ItemPicture {...{showState, data}}/>;
		case "Audio Cloze Item": return <AudioCloze {...{showState, data}}/>;
		case "Audio Cloze Item Picture": return <AudioClozePicture {...{showState, data}}/>;
	}
  }
  return (
    <Card border={isPreview ? "light" : "dark"} className="m-1">
    	{element()}
    {
      isPreview || <ElementButtons {...{setState: setShowState, showState}} />
    }
    </Card>
  );
}
